...
static void writeJson(const nlohmann::json &value, const std::string &path, bool pretty)
{
    ...
    std::ofstream file;
    if (!path.empty()) { file.open(path.c_str()); if (!file) throw std::runtime_error("cannot open output: " + path); out = &file; }
...
int main(int argc, char **argv)
{
    ...
    inspect->add_option("-o,--output", inspectOutput, "Write JSON to this file instead of stdout");
    ...
    try {
        ...
        if (*inspect) writeJson(inspectJson(firmware), inspectOutput, pretty);
