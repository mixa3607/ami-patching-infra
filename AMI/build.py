...
DEFAULT_SCOPES = ("prepare", "logos", "mcodes")
STAGES = {
    "prepare": Stage("prepare", (), prepare.run, ready=True),
    "dmi": Stage("dmi", ("prepare",), dmi.run, ready=False),
    "logos": Stage("logos", ("prepare",), logos.run, ready=True),
    "ifr": Stage("ifr", ("prepare",), ifr.run, ready=False),
...
def main() -> int:
    ...
    parser.add_argument("--scopes", action="append", help="Comma-separated stages to run (default: prepare,logos,mcodes)")
