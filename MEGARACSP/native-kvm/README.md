...
`scripts/build-imb760-image.sh` creates a complete, flashable 32 MiB image from
the verified baseline, preserving every unlisted region. It builds the current
native KVM source into slot A (`0x016d0000`), sets both U-Boot MAC variables,
sets the fixed-size firmware-info record to `MOD N1 VER02` / `RR10 AST2500 MOD
N1 VER 02`, and writes the `bak2shell` `/conf/default_sh` payload into primary,
backup, and failsafe JFFS2 partitions. The baseline is required to have SHA-256
`aa8ae3bf892d18689e9ba7d0089a3abc5b413671aa6673a438f7dbf08df2e68c`.

```sh
scripts/build-imb760-image.sh \
  /path/to/uefi-mod-tools \
  /path/to/IMB760_BMC_mixa3607_F8CC6E033B82_zero-boot.bin \
  /path/to/videocap.ko \
  out/IMB760_BMC_native-kvm.bin
```

Omit `VIDEOCAP.KO` to build an image that changes only the U-Boot MAC variables
and fixed-size firmware-info record:

```sh
scripts/build-imb760-image.sh \
  /path/to/uefi-mod-tools \
  /path/to/IMB760_BMC_mixa3607_F8CC6E033B82_zero-boot.bin \
  out/IMB760_BMC_mac-version.bin
```

The host additionally needs `jq`, `jefferson`, `fakeroot`, `mkfs.jffs2`, and
`jffs2dump`. The builder re-splits the output and verifies both MAC variables,
all three `default_sh` files, and the injected native-KVM slot before reporting
the result. It does not write BMC flash.

Set the two-digit release suffix in the repository-root `version.txt`; the
builder uses it for both `FW_BUILDTIME` (`MOD N1 VERNN`) and `FW_DESC` (`RR10
AST2500 MOD N1 VER NN`).

Set the MAC address in the repository-root `eth-mac.txt`; the builder writes it
to both U-Boot variables, `ethaddr` and `eth1addr`.

`scripts/extract-config.sh PARTITIONS-DIR OUTPUT-DIR` extracts the three
configuration JFFS2 regions, while `scripts/verify-config.sh OUTPUT-DIR
REPORT-DIR` hashes and compares their contents.

## A/B deployment

**Flash writes can permanently brick the BMC. These scripts are specific to the
confirmed erased NOR range below. They never update root or www. Do not infer
other offsets from this layout, and do not run the updater while normal flash
maintenance, boot, or another MTD writer is active.**

```text
slot A  0x016d0000..0x01ad0000  0x400000 bytes  blocks 0x16d..0x1ac
slot B  0x01ad0000..0x01ed0000  0x400000 bytes  blocks 0x1ad..0x1ec
reserve 0x01ed0000..0x01ff0000
```

Copy an already-built image and `scripts/write-slot.sh` to temporary target
storage. Verify its SHA-256 out of band. As root and while the selected slot is
inactive, write it with the literal confirmation flag:

```sh
/tmp/write-slot.sh B /tmp/native-kvm-B.img \
  --i-understand-this-erases-native-kvm-slot
```

The updater requires an exact-size image with little-endian CramFS magic,
erases exactly 64 blocks at the selected offset, writes exactly 64 blocks, and
hashes a full read-back through `/dev/mtd0ro`. It refuses the selected slot when
that slot is enabled and does not activate a successful write.

Install `scripts/native-kvm-bootstrap.sh` as
`/conf/native-kvm-bootstrap.sh`. Integrate its invocation into an existing
writable `/conf` boot hook; do not modify root or www. It does nothing unless
the enable marker exists. Select and enable only after writing and verifying an
inactive slot:

```sh
printf '%s\n' B > /conf/native-kvm.slot
touch /conf/native-kvm.enabled
/conf/native-kvm-bootstrap.sh
```

The bootstrap maps `/dev/mtdblock0` at the exact selected offset through a free
loop device, mounts CramFS read-only at `/var/native-kvm`, validates the bundle,
waits for `adviserd`, and launches the existing recovery-oriented module-swap
wrapper through `start-stop-daemon`. Logs go to `/var/log/native-kvm.log` and
the launcher PID goes to `/var/run/native-kvm.pid`. The wrapper intentionally
stops `adviserd`, swaps `videocap`, and restores both on normal exit; bootstrap
preflight or mount validation failures leave stock services untouched.

### Recovery

To disable native KVM without touching flash, remove the marker and reboot:

```sh
rm -f /conf/native-kvm.enabled
reboot
```

If the selected image does not mount or validate, the bootstrap exits before
stopping stock services. Remove the marker, select the previously verified slot
in `/conf/native-kvm.slot`, re-enable, and reboot. If neither slot is usable,
leave native KVM disabled and boot the stock firmware. Reflash only an inactive
slot and never erase outside the exact slot ranges above. Power loss during an
inactive-slot update should leave the active slot and stock root/www intact.
