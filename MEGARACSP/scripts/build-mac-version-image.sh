...
usage()
{
	...
	printf '       %s --payload ROOT.CRAMFS SLOT.CRAMFS OUTPUT.BIN\n' "$0" >&2
	exit 2
}
...
if [ ! -f "$mac_file" ]; then
	printf '%s\n' 'WARNING: eth-mac.txt is missing, falling back to eth-mac.example.txt' >&2
	mac_file=$workspace/eth-mac.example.txt
fi
[ -f "$mac_file" ] || { printf 'missing MAC file: %s\n' "$mac_file" >&2; exit 1; }
[ "$payload" -eq 0 ] || [ -f "$root_image" ] || { printf 'missing payload root image: %s\n' "$root_image" >&2; exit 1; }
[ "$payload" -eq 0 ] || [ -f "$slot_image" ] || { printf 'missing payload slot image: %s\n' "$slot_image" >&2; exit 1; }
...
mac=$(grep -E -m1 '^[[:xdigit:]]{2}(:[[:xdigit:]]{2}){5}$' "$mac_file" | tr -d '\r\n')
if ! printf '%s\n' "$mac" | grep -Eq '^[[:xdigit:]]{2}(:[[:xdigit:]]{2}){5}$'; then
	printf 'no valid MAC address found in %s\n' "$mac_file" >&2
	exit 1
fi
[ "$payload" -eq 0 ] || [ "$(wc -c < "$root_image")" -eq "$ROOT_SIZE" ] || {
	printf 'unexpected payload root image size: %s\n' "$root_image" >&2
	exit 1
}
[ "$payload" -eq 0 ] || [ "$(wc -c < "$slot_image")" -le "$SLOT_SIZE" ] || {
	printf 'payload slot image %s exceeds the %s-byte partition\n' "$slot_image" "$SLOT_SIZE" >&2
	exit 1
}
...
if [ "$payload" -eq 1 ]; then
	...
	cp "$slot_image" "$tmp/partitions/80_payload-slot-a.bin"
fi
...
if [ "$payload" -eq 1 ]; then
	cmp -s "$root_image" "$tmp/verified/50_root.cramfs" || {
		printf '%s\n' 'payload root verification failed' >&2
		exit 1
	}
	slot_size=$(wc -c < "$slot_image")
	[ "$slot_size" -le "$SLOT_SIZE" ] || {
		printf '%s\n' 'payload slot too large' >&2
		exit 1
	}
	cmp -s -n "$slot_size" "$slot_image" "$tmp/verified/80_payload-slot-a.bin" || {
		printf '%s\n' 'payload slot verification failed' >&2
		exit 1
	}
	[ "$(dd if="$tmp/verified/80_payload-slot-a.bin" bs=1 skip="$slot_size" 2>/dev/null | tr -d '\377' | wc -c)" -eq 0 ] || {
		printf '%s\n' 'payload slot tail is not erased-NOR' >&2
		exit 1
	}
fi
for image in 01_uboot.bin 20_conf-main.bin 30_conf-backup.bin 40_conf-failsafe.bin \
	50_root.cramfs 60_uimage.bin 70_web-data.cramfs 80_payload-slot-a.bin 90_firmware-info.bin; do
	...
	if [ "$payload" -eq 1 ]; then
		case "$image" in
			50_root.cramfs|80_payload-slot-a.bin) continue ;;
		esac
	fi
