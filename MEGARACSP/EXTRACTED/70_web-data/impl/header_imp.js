...
function doInit()
{
	...
	if(top.user.name=='')
	{
		...
		top.user.extendedpriv = top.fnCookie.read("Extendedpriv");
...
function refresh()
{
	...
	for(i = 0; i < tabCtrl.length; i++) {
		if ((tabCtrl[i].className == "classicTab") && 
			...
			tabCtrl[i].style.fontWeight == 700)) {
...
function logout()
{
	if(top.gActiveBMCInst != 1)
	{
		var req = new xmit.getset({url:"/rpc/setbmcinst.asp", 
			onrcv:function(){}, status:""});
		req.add("SELECTED_NODE", 1);
		req.send();
		delete req;
