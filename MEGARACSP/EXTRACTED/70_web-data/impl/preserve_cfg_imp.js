...
// configuration items, which is used for restore configuration.
...
function setPreserveCfgRes(arg)
{
	...
	if(arg.HAPI_STATUS != 0) {
		...
		STATUS_DATA = WEBVAR_JSONVAR_SETPRESERVECFG.WEBVAR_STRUCTNAME_SETPRESERVECFG;
		for(i = 0; i < STATUS_DATA.length; i++) {
			if (STATUS_DATA[i].STATUS != 0) {
				errstr += eLang.getString("common", "STR_NEWLINE");
				errstr += eLang.getString("common","STR_PRSRV_NAME_" + STATUS_DATA[i].SELECTOR);
			}
		}
		alert(errstr);
	} else {
...
function loadPreserveCfg(bopt)
{
	...
	var prsrvStatus;	//DOM Element to hold preserve configuration status
	var prsrvName = ""; //String to hold the preserve configuration item names
	tblPrsrvCfg.clear();

	for(i = 0; i < PRSRVCFG_DATA.length; i++) {
		prsrvStatus = "<input type='checkbox' id='_chkPrsrvStatus" + i + "'>";
		prsrvName = eLang.getString("common","STR_PRSRV_NAME_" + PRSRVCFG_DATA[i].SELECTOR);
		try {
			rowJSON.push({cells:[
