...
function setPHYCfgRes(arg) {
	...
	switch(GET_ERROR_CODE(arg.HAPI_STATUS)) {
		case top.CONSTANTS.SUCCESS:
			...
			break;
		case 0x85:
			alert (eLang.getString("common", "STR_CONF_PHY_SAVE_ERROR"));
			break;
		default:
...
/* This method is used to remove the 1000 Mbps link speed. Because as of now, 
 * its not supported, when auto negotiation is OFF.
 */
function removeLinkSpeed() {
	if (rdoAutoNegOff.checked) {
		for (i = 0; i < lstLinkSpeed.length; i++) {
			if (lstLinkSpeed.options[i].value == 1000) {
				try {
					lstLinkSpeed.options.remove(i);
				} catch(e) {}
				break;
			}
		}
