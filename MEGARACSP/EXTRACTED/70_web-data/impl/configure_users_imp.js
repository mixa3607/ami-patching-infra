...
var CONST_FIXEDUSER_COUNT; //It used to hold the IPMI fixed user count value
...
var DISABLE_EMPTY_PASSWORD = false;		//It used to hold the empty password login support
var userAuthProt;
var userSNMPStatus;
...
function _begin()
{
	...
	DISABLE_EMPTY_PASSWORD = checkProjectCfg("DISABLE_EMPTY_PASSWORD");
			...
			if(this.selected.length)
			{
				...
				if (USERINFO_DATA[selectedid-1].UserName == "") {
					...
					delUserBtn.disabled = (selectedid <= 
						(USERINFO_DATA[selectedid-1].FixedUserCount)) ? true : 
						false;
				}
...
function getAllUserInfoRes (arg)
{
	if (GET_ERROR_CODE(arg.HAPI_STATUS) == 0xD4) {	//Insufficient privilege level
		...
		CONST_FIXEDUSER_COUNT = USERINFO_DATA[0].FixedUserCount;
		...
		for (i = 0; i < USERINFO_DATA.length; i++) {
			...
			if (safari) {
				usernametodisplay = usernametodisplay.replace(/\&/g, "&amp;");
			}
...
function frmAddUser(arg)
{
	...
	userAccess = frm.addCheckBox(eLang.getString("common", 
		"STR_CONF_USER_ACCESS"), "_userAccess",{"_chkUserAccess":"Enable"},
		false,[""]);
	userAccess = userAccess._chkUserAccess;
...
*/
	nwPrivHead = eLang.getString("common", "STR_CONF_USER_NWPRIV");
	nwPrivRow = frm.addRow(nwPrivHead, bindUsersList("_nwPriv", 
		"classicTxtBox"));
	...
	fileupload.innerHTML = "<form name='sshUpload' id='_sshUpload' " +
		"method='POST' enctype='multipart/form-data' " +
		"action='file_upload.html?SOURCE=SSHKey' target='hiddenFrame' " +
		"style='margin-bottom:0'>" +
		"<input type='file' name='sshkey' id='_sshFileBrowse' " +
		"size='35' />" +
		"</form>";
...
function validateUserCfg()
{
	...
	var err = 0;
	var checkpword = DISABLE_EMPTY_PASSWORD;
	var varPwordSize = 1;

	//Username validation
	if (!eVal.username(userName.value, "IPMI", 1, 16)) {
	//2nd argument "IPMI" is to select the specific filter in eVal
		err = 1;
		errmsg += "* " + eLang.getString("common", "STR_CONF_USER_NAME") +
	...
	if (checkpword) {
		if ($("byte16").checked) {
			if (!eVal.password(passWord.value, varPwordSize, 16)) {
				err = 1;
				errmsg += "* " + eLang.getString("common", "STR_PASSWORD") +
					eLang.getString("common", "STR_NEWLINE");
			}
		} else if ($("byte20").checked) {
			if (!eVal.password(passWord.value, varPwordSize, 20)) {
				err = 1;
				errmsg += "* " + eLang.getString("common", "STR_PASSWORD") +
					eLang.getString("common","STR_NEWLINE");
			}
			...
			errmsg += "* " + eLang.getString("common",
				"STR_CONF_USER_PWORDSIZE") + eLang.getString("common",
				"STR_NEWLINE");
		}
		...
		if (passWord.value != cpassWord.value) {
			err = 1;
			errmsg += "* " + eLang.getString("common",
				"STR_CONF_USER_CNFMPWORD") + eLang.getString("common",
				"STR_NEWLINE");
		}
	}
...
function setUserInfo()
{
	...
	req.add("FixedUserCount", CONST_FIXEDUSER_COUNT);
	...
	if (EXTENDED_PRIV) {
		req.add("KVMPriv", chkKVMPriv.checked ? 1 : 0);
		req.add("VMediaPriv", chkVMediaPriv.checked ? 1 : 0);
	}
...
function initFrmModifyUser(selectindex)
{
	...
	if (!SNMP_SUPPORT) {
		frmModifyUser({"userindex"	: selectindex,
			...
			"KVMPriv"		: selectUserRow.KVMPriv,
			"VMediaPriv"	: selectUserRow.VMediaPriv,
...
function frmModifyUser(arg)
{
	...
	userName = frm.addTextField(eLang.getString("common","STR_CONF_USER_NAME"),
		"_userName",arg.username,{"readOnly":(((arg.userindex) > CONST_FIXEDUSER_COUNT) ? false : true)},"classicTxtBox");
	...
	userAccess = frm.addCheckBox(eLang.getString("common", 
		"STR_CONF_USER_ACCESS"), "_userAccess",{"chkUserAccess":"Enable"}, 
		false,[""]);
	userAccess = userAccess.chkUserAccess;
...
*/
	...
	if (SNMP_SUPPORT) {
		chkSNMPStatus = frm.addCheckBox(eLang.getString("common", 
			"STR_CONF_SNMP_STATUS"), "_chkSNMPStatus", {"_chkSNMPStatus" : "Enable"},
			false, [""], {"disabled" : arg.username == "anonymous" ? true : false});
		chkSNMPStatus = chkSNMPStatus._chkSNMPStatus;
		chkSNMPStatus.onclick = enableSNMP;
	...
	var fileupload = document.createElement("div");
	fileupload.innerHTML = "<form name='sshUpload' id='_sshUpload' " +
		"method='POST' enctype='multipart/form-data' target='hiddenFrame'" +
		"action='file_upload.html?SOURCE=SSHKey'  style='margin-bottom:0'>" +
		"<input type='file' name='sshkey' id='_sshFileBrowse' size='35' />" +
		"</form>";
