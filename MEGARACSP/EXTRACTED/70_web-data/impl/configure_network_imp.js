...
var LANSTATUS = {			//It holds LAN Enable status
	    NONE: 0,
	    IPv4: 1,
	    IPv6: 2,
	    BOTH:3
...
function _begin()
{
	fillLanChannel();
	getAllLANCfg();
	if(!checkProjectCfg("IPV6_COMPLIANCE")){
		trV6Gateway.className = "visibleRow";
	}
}
...
function reloadLANInfo()
{
	...
	chkLANEnable.checked = (LANCFG_DATA[index].lanEnable) ? true : false;
	...
	var IsEnable= false;
	if(LANCFG_DATA[index].lanEnable==LANSTATUS.BOTH) {
		chkV4Enable.checked=true;
		chkV6Enable.checked=true;
	} else if(LANCFG_DATA[index].lanEnable==LANSTATUS.IPv6) {
		chkV4Enable.checked=false;
		chkV6Enable.checked=true;
	} else if(LANCFG_DATA[index].lanEnable==LANSTATUS.IPv4) {
		chkV4Enable.checked=true;
		chkV6Enable.checked=false;
	} else if(LANCFG_DATA[index].lanEnable==LANSTATUS.NONE) {
		chkV4Enable.checked=false;
		chkV6Enable.checked=false;
	}
...
{
	if(chkLANEnable.disabled == false) {

		var eth0valueIndex=getIndex(lstLANInterface.options[0].value);
		var eth0IndexValue=LANCFG_DATA[eth0valueIndex].lanEnable
		var eth1valueIndex=getIndex(lstLANInterface.options[1].value);
		var eth1IndexValue=LANCFG_DATA[eth1valueIndex].lanEnable;
		var Index=getIndex(lstLANInterface.value);
		var Indexvalue= LANCFG_DATA[Index].lanEnable;

		if(Index > 0) {
			if( eth0IndexValue == 0 && chkLANEnable.checked== false) {
				alert (eLang.getString("common", "STR_CONF_NW_INTERFACE_DISABLE"));
				return; 
			}             
		}
		else {
			if( eth1IndexValue == 0 && chkLANEnable.checked== false) {
				alert (eLang.getString("common", "STR_CONF_NW_INTERFACE_DISABLE"));
				return; 
			}             
		}
	...
	if (chkLANEnable.checked) {
		if(chkV4Enable.checked== false && chkV6Enable.checked== false)	{
			alert (eLang.getString("common", "STR_CONF_NW_IP4_IP6_ENABLE"));
			return;			
		}
	}
	...
	if(chkV4Enable.checked) {
		...
		if(chkV6Enable.checked) {
			if (!chkV6IPSource.checked) {			//V6 DHCP
				...
				if(!checkProjectCfg("IPV6_COMPLIANCE")) {
					if(!eVal.ipv6(txtV6Gateway.value, true, true)) {
		...
		if(chkVLANEnable.checked) {
			if(!eVal.isnumstr(txtVLANId.value, 2, 4094)) {
				alert (eLang.getString("common", "STR_CONF_NW_INVALID_VLANID") +
					eLang.getString("common", "STR_HELP_INFO"));
				txtVLANId.focus();
				return;
			}
			if(!eVal.isnumstr(txtVLANPriority.value, 0, 7)) {
				alert (eLang.getString("common", 
...
function setNetworkInfo()
{
	if(confirm(eLang.getString("common", "STR_CONF_NW_SAVE_CONFIRM"))) {
		...
		var NewLANEnable;
		if(chkLANEnable.checked == true) {
			if(chkV4Enable.checked==true && chkV6Enable.checked==true) {
				NewLANEnable=LANSTATUS.BOTH;	
				} else if(chkV4Enable.checked==false && chkV6Enable.checked==true) {
				NewLANEnable=LANSTATUS.IPv6;
				} else if(chkV4Enable.checked==true && chkV6Enable.checked==false) {
				NewLANEnable=LANSTATUS.IPv4;
				}
			} else {
			NewLANEnable=LANSTATUS.NONE;
		}		

			var newLANEnable  =NewLANEnable;// (chkLANEnable.checked) ? 1 : 0;
...
/*
 * It will disable or enable the v4 user controls based on Enable check box of
 * v4 settings.
 */
function enableV4()
{
	chkV4IPSource.disabled = (chkV4Enable.disabled || (!chkV4Enable.checked));
	DHCPEnable((chkV4Enable.disabled || chkV4IPSource.disabled || 
		chkV4IPSource.checked), "V4");
}
...
{
	var bopt;		//boolean value to enable/disable
	bopt = (chkVLANEnable.disabled || (!chkVLANEnable.checked));
	var index = 0;		//index of the global RPC response data
	index = getIndex(lstLANInterface.value);
	if(bopt==true) 	{
		txtVLANId.value=0;
		txtVLANPriority.value=0;
		chkVLANEnable.checked=false;
	} else	{
		txtVLANId.value = LANCFG_DATA[index].vlanID;
		txtVLANPriority.value = LANCFG_DATA[index].vlanPriority;
	}	
