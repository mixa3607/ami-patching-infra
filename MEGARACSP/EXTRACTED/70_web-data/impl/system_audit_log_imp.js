...
function getSystemLog()
{
	...
	lblHeader.innerHTML = "<strong class='st'>" +
		eLang.getString("common", "STR_SYSTEM_LOG_CNT") +
		"</strong> 0 " + eLang.getString("common", "STR_EVENT_ENTRIES");

	var req = new xmit.getset({url:"/rpc/getsystemlog.asp",
		onrcv:getSystemLogRes, status:""});
...
function getSystemLogRes(arg)
{
	if (arg.HAPI_STATUS != 0) {
		...
		fillListGrid(WEBVAR_JSONVAR_GETSYSTEMLOG.WEBVAR_STRUCTNAME_GETSYSTEMLOG,
			"STR_SYSTEM_LOG_CNT");
	}
}
...
function getAuditLogRes(arg)
{
	...
	{
		fillListGrid(WEBVAR_JSONVAR_GETAUDITLOG.WEBVAR_STRUCTNAME_GETAUDITLOG,
			"STR_SYSTEM_LOG_CNT");
	}
}
...
function fillListGrid(data, str)
{
	...
	if (data.length > 0) {
		for (i = 0; i < data.length; i++) {
			if (data[i].SYSTEM_LOG != undefined) {
				rec_data = data[i].SYSTEM_LOG.split(" ");
			} else if (data[i].AUDIT_LOG != undefined) {
				rec_data = data[i].AUDIT_LOG.split(" ");
			}
			if(rec_data.length > 4) {
				date_col = (rec_data[1].length == 0) ? 2 : 1;
				tmp = rec_data[1].length;
				if((rec_data[date_col + 1].split(":").length == 3) &&
					(rec_data[0].length == 3)) {
					eventid++;
					timestr = rec_data[0] +
						eLang.getString("common", "STR_EMPTY") + 
						rec_data[date_col] +
						eLang.getString("common", "STR_EMPTY") + 
						card_year + eLang.getString("common", "STR_EMPTY") +
						rec_data[date_col + 1];

					event_milliseconds = Date.parse(timestr); //Date in milliseconds
					//The log entires having the local time itself, so no need to add the UTC Offset here
					...
					timestamp += "  " + disp_hours + ":" + disp_mins + ":" + 
						disp_secs;
	...
	logTable.loadFromJson(tblJSON);
	lblHeader.innerHTML = "<strong class='st'>" +
		eLang.getString("common", str) + "</strong>" + eventid +
