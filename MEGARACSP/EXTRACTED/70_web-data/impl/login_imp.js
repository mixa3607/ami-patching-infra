...
var LOGIN_DATA;    //Global Variable that used to hold the Login RPC data
...
function validate(frm)
{
	...
	// The password validation should be skip, because project may have empty password support
	/*
...
function loginRes(arg)
{
	...
	logincalled = false;
	if ((fnCookie.read("SessionCookie") != undefined && fnCookie.read("SessionCookie") != null)){
                top.location.href = "/index.html";
        }
        else{
		LOGIN_DATA = WEBVAR_JSONVAR_WEB_SESSION.WEBVAR_STRUCTNAME_WEB_SESSION[0];
		if (window.location.protocol === "https:") {
			top.document.cookie = "SessionCookie=" + LOGIN_DATA.SESSION_COOKIE +
				"; path=/; secure";
		} else {
			top.document.cookie = "SessionCookie=" + LOGIN_DATA.SESSION_COOKIE + "; path=/";
		}
		top.document.cookie = "SESSION_ID=" + LOGIN_DATA.SESSION_ID + "; path=/";
		top.document.cookie = "BMC_IP_ADDR=" + LOGIN_DATA.BMC_IP_ADDR + ";path=/";
		top.user.CSRFtoken = LOGIN_DATA.CSRFTOKEN;
	}
	var lang;	//Browser Language
	lang = top.CONSTANTS.DEFAULT_LANG;
	if (navigator.userLanguage != undefined) {
		lang = navigator.userLanguage;
	} else if (navigator.language != undefined) {
		lang = navigator.language;
	} else {
		alert(eLang.getString("common", "STR_LANG_NOT_FOUND"));
	}

	lang = lang.split("-");
	lang = lang[0].toUpperCase();
	top.document.cookie = "Language=" + lang + ";path=/";
...
function getLanInfo(arg)
{
	...
	if(arg.HAPI_STATUS == 0) {
		...
		if (userDetails.CURPRIV != top.CONSTANTS.NOACCESS) {
			top.document.cookie = "Username=" + userDetails.CURUSERNAME + ";path=/";
			top.document.cookie = "PNO=" + userDetails.CURPRIV + ";path=/";
			if (userDetails.EXTENDED_PRIV != undefined && 
				userDetails.EXTENDED_PRIV != null) {
				top.document.cookie = "Extendedpriv=" + 
					userDetails.EXTENDED_PRIV + ";path=/";
			}
			...
			top.user.extendedpriv = userDetails.EXTENDED_PRIV;
...
function getLanInfoRes(arg)
{
	...
	var eth_enable = [];	//lan enable value
	...
	case 0x0:
		...
		for (i = 0; i < LANINFO.length; i++) {
			...
			eth_enable.push(LANINFO[i].INTERFACE_ENABLE);
		}
		...
		gSettingsCookieStr.push("enable:[" + eth_enable.join(",") + "]");
	break;
...
/*
 * This method is used to get all services license information status.
 * getLicenseCfgRes method is used to give the response status of the RPC call.
 * Need to check HAPI_STATUS, intimate end user if it returns non-zero value.
 * If success, move the License data to the cookie variable.
 * @param fnptr function, specifies the method to invoke the operations.
 */
function getLicenseCfg(fnptr) {
	xmit.get({url:"/rpc/getlicenseinfo.asp", onrcv:getLicenseCfgRes, status:""});

	function getLicenseCfgRes(arg) {
		var strLicense = ""; //String to hold the Un-licensed services
		if (arg.HAPI_STATUS == 0) {
			LICENSE = WEBVAR_JSONVAR_GETLICENSECFG.WEBVAR_STRUCTNAME_GETLICENSECFG;
			for (index = 0; index < LICENSE.length; index++) {
				if ((LICENSE[index].SERVICE_NAME != "") && 
					(LICENSE[index].VALIDITY == 0)) {
					strLicense += LICENSE[index].SERVICE_NAME + ",";
				}
			}
		}
...
function forgotPasswordRes(arg)
{
	...
	switch (returncode)
	{
		...
		break;
		case 0x11:
			alert (eLang.getString("common","STR_SMTP_SERVER_DISABLED"));
		break;
		case 0x12:
		case 0x13:
		case 0x14:
