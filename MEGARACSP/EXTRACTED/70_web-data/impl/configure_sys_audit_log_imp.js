...
function setLogCfg()
{
	if (top.user.isAdmin()) {
			...
			if (localLog.checked) {
				if (!eVal.isnumstr(fileSize.value, 3, 65535)) {
					alert(eLang.getString("common", 
						"STR_CONF_SYS_AUDIT_INVALID_FILESIZE") + 
				...
				if (!eVal.isnumstr(rotateCount.value, 0, 255)) {
					alert(eLang.getString("common",
						"STR_CONF_SYS_AUDIT_INVALID_ROTATECNT") +
						eLang.getString("common", "STR_HELP_INFO"));
					rotateCount.focus();
					return;
				}
			} else if(remoteLog.checked) {
				if ((!eVal.domainname(serverAddr.value,true)) && 
					(!eVal.ip(serverAddr.value,false)) && 
					(!eVal.ipv6(serverAddr.value, false, false))) {
				...
				alert (eLang.getString("common",
					"STR_CONF_SYS_AUDIT_INVALID_LOGTYPE"));
				localLog.focus();
				return;
			}
		}
		...
		req.add("SYSLOGENABLE", (enableSystemLog.checked ? 
			(localLog.checked ? 1 : (remoteLog.checked ? 2 : 0)) : 0));
...
function setLogCfgRes(arg)
{
	if(arg.HAPI_STATUS != 0) {
		...
		case 0xDF: case 0xE2: case 0xE4:
			...
			break;
		case 0x83:
			alert(eLang.getString("common", "STR_INVALID_ADDR_FAMILY"));
			serverAddr.focus();
			break;
		default:
