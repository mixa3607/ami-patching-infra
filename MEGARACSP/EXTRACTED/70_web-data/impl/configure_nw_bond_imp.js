...
var BOND_SUPPORT;
var bondEnable = 0;
var CONST_ACTIVE_BACKUP = 0x1;	//It used to hold the Active backup value
...
function reloadNWBondCfg()
{
	...
	txtBondMode.value = (NWBONDCFG_DATA.BOND_MODE >= 0 && 
		NWBONDCFG_DATA.BOND_MODE <= 6) ? eLang.getString("common", 
		"STR_NW_BOND_MODE_" + NWBONDCFG_DATA.BOND_MODE) : 
		eLang.getString("common", "STR_NW_BOND_MODE_0");
	if (NWBONDCFG_DATA.BOND_MODE != CONST_ACTIVE_BACKUP) {
		lstBondIfc.add(new Option(eLang.getString("common", 
			"STR_NW_BOND_IFC_3"), 3), isIE?index:null);
	}
	lstBondIfc.value = fillSelectBox(lstBondIfc, NWBONDCFG_DATA.BOND_IFC);
	chkAutoConf.checked = NWBONDCFG_DATA.AUTO_CONF ? true : false;	
	enableNWBond();
}
...
function enableNWBond()
{
	...
	chkAutoConf.disabled = (NWBONDCFG_DATA.BOND_ENABLE == chkEnableBond.checked);
}