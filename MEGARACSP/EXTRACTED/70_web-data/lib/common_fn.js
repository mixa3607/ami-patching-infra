...
{
	//alert("in common createWebEventsRecord");
...
{
	//alert("in common createEventGraph");
	if(fresh)
	{
...
/*RAID COntroller graph*/

function createRAIDWebEventsRecord(gSettings,ctrlId)
{
	//alert("in common createRAIDWebEventsRecord");
	//top.reloadValues is a global set to true when page refreshes. See header_imp.js - Manoj (Bug : 11734)
	if((gRAIDEventData = parent.gRAIDEventData)==undefined || top.reloadValues) 
	{
		//alert("in common gRAIDEventData = parent.gRAIDEventData");
	...
	for(var i=0; i<gRAIDEventData.length;i++)
	{
		...
		if(ctrlId==gRAIDEventData[i].CTRLID) {
			//alert("in ctrlId==gRAIDEventData[i].CTRLID");
	...
	//alert(graphData);
	gSettings.gData=graphData;
	createRAIDEventGraph(gSettings,false,ctrlId);
}

function createRAIDEventGraph(gSettings, fresh,ctrlId)
{
	//alert("in common createRAIDEventGraph");
	//alert(ctrlId);
	if(fresh)
	{
	...
	if(gSettings.gData==undefined || gSettings.gData==null)
	{
		//alert("in common gSettings.gData");
		createRAIDWebEventsRecord(gSettings,ctrlId);
		return;
	}
...
function rebootDevice(FLASH_SUCCESS, DEVICE_RESET, UPDATE_CANCEL) {
	xmit.get({url:(FLASH_SUCCESS == true) ? "/rpc/postflashreset.asp" : 
		"/rpc/rebootcard.asp", onrcv:function(){}, status:''});
	if (DEVICE_RESET) {
		MessageBox(eLang.getString("common","STR_DEVICE_RESET_TITLE"), 
			p(eLang.getString("common","STR_DEVICE_RESET_DESC")), [], true);
	} else if (UPDATE_CANCEL) {
		MessageBox(eLang.getString("common","STR_DEVICE_UPDATE_CANCEL_TITLE"), 
			p(eLang.getString("common","STR_DEVICE_UPDATE_CANCEL_DESC")), [], true);
	} else {
...
function restartHTTPSService(alertstr)
{
	...
	if (window.location.href.indexOf("https:") != -1) {
		prepareDeviceShutdown();
		var mb = MessageBox(eLang.getString("common","STR_WEB_RESET_TITLE"), 
			p(eLang.getString("common", "STR_HTTPS_RESET_DESC")), [], true);
		return true;
	} else {
...
/*
 * This function is used to remove duplicate options added to the list box.
 * @param lstBoxID object, ID of the list box object.
 */
function removeDuplicate(lstBoxID)
{
	...
	if (lstBoxID.options.length > 0) {
		for (i = 0; i < lstBoxID.options.length - 1; i++) {
			for (j = i + 1; j < lstBoxID.options.length;) {
				if (lstBoxID.options[i].value == lstBoxID.options[j].value) {
					lstBoxID.remove(j);
				} else {
					j++;
				}
...
/*
 * This function is used to get listbox index for the given list box value.
 * @param lstBoxID object, ID of the list box object.
 * @param lstBoxValue object, value in the list box.
 */
function getListIndex(lstBoxID, lstBoxValue)
{
	var i;		//loop counter
	var index = -1;		//index of the list box
	if (lstBoxID.options.length > 0) {
		for (i = 0; i < lstBoxID.options.length; i++) {
			if (lstBoxID.options[i].value == lstBoxValue) {
				index = i;
				break;
			}
		}
		if (index == -1) {
			alert ("Error in getting index for list value" + lstBoxValue);
			index = 0;
		}
	} else {
		alert ("No Elements available in list box");
		index = 0;
	}
	return index;
}
...
/*
 * This function is used to check the license configuration for the given 
 * feature. If the feature is present in the Project configuration list
 * and given service is licensed, then it returns true. Otherwise it 
 * returns false.
 * @param service string, service name to be searched in the list.
 * @return boolean, true-if service license exists, false-not exists.
 */
function checkLicenseCfg(service) {
	if (top.settings.features.indexOf("RUNTIME_LICENSE") != -1) {
		eval("licenseCfg = " + top.fnCookie.read("License"));
		if (licenseCfg != undefined && licenseCfg != null) {
			if (licenseCfg.indexOf(service) != -1) {
				return false;
			} else {
				return true;
			}
...
/*
 * This function is used to assign the default select value for selectBox.
 * Once selectIndex value is not match with the Index value in the selectBox,
 * this will return the very first value of selectBox value as default selectvalue.
 * @param selectBox object, contains list of value available in the selectBox.
 * @param selectIndex number, spcifies index value for selectBox.
 * @return number, holds the selectIndex for particular selectBox.
 */
function fillSelectBox(selectBox, selectIndex)
{
	for (var j = 0; j < selectBox.length; j++) {
		if (selectBox[j].value == selectIndex) {
			return selectIndex;
		}
	}
	selectIndex = selectBox[0].value;
	return selectIndex;
}
/* 
 * This method is used to bind the selectBox values.
 * @param id string, holds the id of the selectBox.
 * @param className string, holds the classname for the selectBox.
 */
function bindUsersList(id, className) {
	var userList = "";
	userList = "<select id=" + id + " name=" + id + " classname=" + className + ">" +
		"<option value='4'>Administrator</option>" +
		"<option value='3'>Operator</option>" +
		"<option value='2'>User</option>" +
		"<option value='5'>OEM Proprietary</option>" +
