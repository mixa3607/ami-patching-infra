...
/* class functions */
	...
	{
		window.onresize = oMenu.closeAllNavigations;
	...
	enableFeaturesList		:	function(oNav)
	{
		if ((top.settings.features != undefined) && (top.settings.features.length > 0)){
			for(var i = 0; i < oNav.items.length; i++)
			{
				...
				if (checkProjectCfg("RUNTIME_LICENSE")) {
					for (service in top.licensePages) {
						for (var j=0; j<top.licensePages[service].length; j++) {
							
							if(service == 'MEDIA'){
								if (oNav.items[i].label == top.licensePages[service][j]) {
									if ((top.fnCookie.read("License") != null) &&
										(top.fnCookie.read("License").indexOf(","+service+",") != -1)) {
										oNav.items[i].enabled = 0;
									}
								}
							}
							else if(service == 'LMEDIA' ||  service == 'RMEDIA'){
								if (oNav.items[i].label == top.licensePages[service][j]) {
									if (top.fnCookie.read("License") != null){
										if((top.fnCookie.read("License").indexOf('LMEDIA') == -1) || (top.fnCookie.read("License").indexOf('RMEDIA') == -1)){
											oNav.items[i].enabled = 1;
										}
										else
										{
				...
				if(checkProjectCfg("SINGLE_NIC_BMC_INSTANCES"))
				{
					...
					{
						if((top.nodePages.indexOf(oNav.items[i].label) != -1) && (top.sharedMenu.indexOf(oNav.items[i].label) == -1) )
							oNav.items[i].enabled = 0;
					}
					else
					{
	...
	/*
	 * This will hide/show the web preview if it is present.
	 * @param className string, className to show or hide.
	 */
	toggleBSODScreen : function(className)
	{
		var BSODScreen = null;
		if ((top.mainFrame.pageFrame.document.getElementById("_rowBSODScreen") != null) && 
			(top.mainFrame.pageFrame.document.getElementById("_rowBSODScreen") != undefined)) {
			BSODScreen = top.mainFrame.pageFrame.document.getElementById("_rowBSODScreen");
		} else if ((top.mainFrame.pageFrame.document.getElementById("_rowWebPreview") != null) && 
			(top.mainFrame.pageFrame.document.getElementById("_rowWebPreview") != undefined)) {
			BSODScreen = top.mainFrame.pageFrame.document.getElementById("_rowWebPreview");
		}
		if ((BSODScreen != null) || (BSODScreen != undefined)) {
			BSODScreen.className = className;
		}
	},
	...
	menu				:	function(name)
	{
		...
		var noItemsDisplay; //to get the no of Items that could be loaded within the height of the resized page.
		var displayItemCountWebPreview;	//Hold the number of items, that could display before the webpreview or BSOD screen
		var start; //to hold the starting index of the menu items.
		var end; //to hold the ending index of the menu items.
		var displayItemCount = 0; //This used to hold total number of enabled feature in top menu
		...
		if(document.getElementById(name+"_menu") == null) {
			...
			var mObj = eval(name);
			noItemsDisplay = (parseInt(((document.body.offsetHeight) - 
				(top.mainFrame.document.getElementById("_menuHolder").offsetHeight)) / 30)); 
			...
			if (noItemsDisplay < oMenu.menuSize) {
				noItemsDisplay = oMenu.menuSize; //Minimum no of Items loaded is 3.
			}
			...
			if ((top.mainFrame.pageFrame.document.getElementById("_tblWebPreview") != null) && 
				(top.mainFrame.pageFrame.document.getElementById("_tblWebPreview") != undefined)) {
				displayItemCountWebPreview = parseInt(top.mainFrame.pageFrame.document.getElementById("_tblWebPreview").offsetTop / 30);
			} else if ((top.mainFrame.pageFrame.document.getElementById("_tblBSOD") != null) && 
				(top.mainFrame.pageFrame.document.getElementById("_tblBSOD") != undefined)) {
				displayItemCountWebPreview = parseInt(top.mainFrame.pageFrame.document.getElementById("_tblBSOD").offsetTop / 30);
			}

			if (displayItemCountWebPreview != null && displayItemCountWebPreview != undefined) {
				if (displayItemCount > displayItemCountWebPreview) {
					oMenu.toggleBSODScreen("hiddenRow");
				}
			}

			if (displayItemCount > noItemsDisplay) {
			...
			{
				if (start == 1) {
					divMenuUp.style.cursor = "default";
					divMenuUp.onclick = function(){};
				} else {
					divMenuUp.style.cursor = "pointer";
					divMenuUp.onclick = moveUp;
				}

				if (end == (oDOMNav.childNodes.length - 2)) {
					divMenuDown.style.cursor = "default";
					divMenuDown.onclick = function(){};
				} else {
					divMenuDown.style.cursor = "pointer";
	...
	closeAllNavigations	:	function()
	{
		...
		oMenu.toggleBSODScreen("visibleRow");
	},
