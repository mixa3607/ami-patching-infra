...
var web_alerts =
{
	...
	reload : function()
	{
		...
		web_alerts.tblJSON = {cols:[
			...
			{text:eLang.getString("common", "STR_EVENT_LOG_HEAD3"), w:"21%", textAlign:"center"},
			{text:eLang.getString("common", "STR_EVENT_LOG_HEAD4"), w:"15%", textAlign:"center"},
			{text:eLang.getString("common", "STR_EVENT_LOG_HEAD5"), w:"35%", textAlign:"center"}
			]};
	 ...
	 */
	toggleWebApplet : function(appletID, className)
	{
		var webApplet = null;
		webApplet = top.mainFrame.pageFrame.document.getElementById(appletID);
		if ((webApplet != null) || (webApplet != undefined)) {
			webApplet.className = className;
		}
	},
	...
	display : function()
	{
		web_alerts.toggleWebApplet("_rowWebPreview", "hiddenRow");
		web_alerts.toggleWebApplet("_rowBSODScreen", "hiddenRow");
		web_alerts.viewCount = 0;
		var para = document.createElement("div");
		para.innerHTML = "<p>" + eLang.getString("common",
			"STR_WEB_ALERT_DESC") + "</p><strong>" + eLang.getString("common",
			"STR_WEB_ALERT_LOG_TITLE") + "</strong>" +
		...
		btns.push(createButton("web_alert_viewall", eLang.getString("common",
			"STR_WEB_ALERT_SEL_VIEW"), web_alerts.viewAllEvents));
		web_alerts.descframe = MessageBox(eLang.getString("common",
			"STR_WEB_ALERT_TITLE"), para, btns);
		web_alerts.descframe.onclose = function() {
			web_alerts.toggleWebApplet("_rowWebPreview", "visibleRow");
			web_alerts.toggleWebApplet("_rowBSODScreen", "visibleRow");
		};
