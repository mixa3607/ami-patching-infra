...
var graph = {
	...
	*/
	...
	{	
		...
		if(this.settings.IsRAID==undefined) {
			this.dataCanvas.style.left=elmOffset(this.canvas,'Left');
			this.dataCanvas.style.top =elmOffset(this.canvas,'Top');
		} else if(this.settings.IsRAID==true) {
			this.dataCanvas.style.left = elmOffset(this.canvas,'Left') + "px";
			this.dataCanvas.style.top = elmOffset(this.canvas,'Top') + "px";
		}
	...
	*/
	drawPieChart			: function(radius)
	{
		...
		for(var i=0; i<gData.length; i++)
		{
			...
			if (gData[i].total_events == gMaxEventData) {
				gData[i].sector_percentage += (100 - totalPercentage);
				totalPercentage = 100;
			}
			startangle += endangle;
...
function getHTMLLegend(color,name,percentage,link)
{
	...
	if(link==undefined)
		...
		htmllegend = "<a href='#' eventType='"+ link +"' onclick='NavigateToEventLog(this);' class='smallColor' style='background:"+color+";cursor:pointer'>&nbsp;&nbsp;&nbsp;</a> "+name+" ("+(Math.round(percentage*100)/100)+"%)";
	else 
