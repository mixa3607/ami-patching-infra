				...
				try{
					if (datao.xmldoc.status!=200)
					{
						...
						if (datao.xmldoc.status==404)
						{
							...
							{
								//this is has been commented for HPM flash
						...
						// to check network related error while operation is ongoing. 
						if ((datao.xmldoc.status == 0) || ((window.ActiveXObject) &&
							(datao.xmldoc.status == 12029))) {
							datao.onerror();
							return;
						}
...
//					code.
			...
			try
			{
				if (hapival != null && hapival != undefined) {
					datao.onrcv({timeout:false,
								url:datao.url,
								id:datao.idnum,
								optarg:datao.optarg,
