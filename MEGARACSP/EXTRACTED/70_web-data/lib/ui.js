...
var validateSessResp = function(arg)
{
	...
	{
		top.gLogout = 1;
		top.location.href = "/page/session_expired.html";
	}
	else
	{
		try
...
/*
 * This function is used to get the CSRFToken when session cookie is valid.
 * @param fnPtr, function reference.
 */
function getCSRFToken(fnPtr) {
	var getCSRFResp = function(arg) {
		if (arg.HAPI_STATUS) {
			top.gLogout = 1;
			top.location.href = "/page/session_expired.html";
		} else {
			try {
				top.user.CSRFtoken = WEBVAR_JSONVAR_WEB_CSRFTOKEN.WEBVAR_STRUCTNAME_WEB_CSRFTOKEN[0].CSRFToken;
			} catch(e) {
				top.user.CSRFtoken = "";
			}
		}
	...
	if (window.location.href.indexOf('http')!= -1) {
		if (window.location.pathname.indexOf('oem') != -1) {	//OEM Feature
			xmit.get({url:"../../../../rpc/WEBSES/getcsrftoken.asp",onrcv:getCSRFResp, status:''});
		} else {
			xmit.get({url:"/rpc/WEBSES/getcsrftoken.asp",onrcv:getCSRFResp, status:''});
		}
	}
...
function clearCookies()
{
	...
	top.fnCookie.erase("BMC_IP_ADDR");
	top.fnCookie.erase("settings");
	top.fnCookie.erase("Username");
	top.fnCookie.erase("Privilege");
	top.fnCookie.erase("Extendedpriv");
	top.fnCookie.erase("PNO");
	top.fnCookie.erase("SessionExpired");
	top.fnCookie.erase("SESSION_ID");
	top.fnCookie.erase("BMC_IP_ADDR");
	top.fnCookie.erase("License");
	top.fnCookie.erase("gMultiLAN");
	top.fnCookie.erase("Language");
	top.fnCookie.erase("test");
}
